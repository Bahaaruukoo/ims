        ...
        if request.user.is_authenticated:
            ...
            if tenant and not getattr(request.user, "is_platform_admin", False):
                if request.user.tenant_id != tenant.id:
                    logout(request)
                    return HttpResponseForbidden("You do not have access to this account. Make sure to log in from the correct domain.")
    ...
    def form_valid(self, form):
        ...
        if tenant:
            if user.tenant_id != tenant.id:
                logout(request)
                messages.error(request, "You do not have access to this account. Make sure to log in from the correct domain.")
        ...
        if not user.tenant_id:
            logout(request)
            messages.error(request, "You are not assigned to any account.")
            return redirect("account_login")
